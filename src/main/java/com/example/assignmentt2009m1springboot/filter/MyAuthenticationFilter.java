package com.example.assignmentt2009m1springboot.filter;

import com.example.assignmentt2009m1springboot.entity.dto.AccountLoginDto;
import com.example.assignmentt2009m1springboot.entity.dto.CredentialDto;
import com.example.assignmentt2009m1springboot.util.JwtUtil;
...
public class MyAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public void successfulAuthentication(HttpServletRequest request, HttpServletResponse response,
        ...
        String accessToken = JwtUtil.generateToken(user.getUsername(), user.getAuthorities().iterator().next().getAuthority(),
                request.getRequestURL().toString(), JwtUtil.ONE_DAY * 7);
        String refreshToken = JwtUtil.generateToken(user.getUsername(), user.getAuthorities().iterator().next().getAuthority(),
